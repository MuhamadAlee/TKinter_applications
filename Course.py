...
try:
    con.execute("CREATE TABLE IF NOT EXISTS course(id INTEGER PRIMARY KEY   AUTOINCREMENT, title TEXT NOT NULL,  section TEXT NOT NULL, code TEXT NOT NULL UNIQUE);")
except sqlite3.Error as er:
    messagebox.showinfo("Error", "Something went wrong")


#insertion of Course record to the database

def insert_data(title,section, code):
    conn = sqlite3.connect("ACME.db")
    flag=True
    try:    
        conn.execute("INSERT INTO course(title,section,code) VALUES( '" + title + "', '" + section +
                 "', '" + code + "' );")
        conn.commit()
    except sqlite3.Error as er:
        messagebox.showinfo("Error", "Something went wrong") 
...
def insert():
    add_window = tk.Tk()
    add_window.title("Insert Course Details")
    add_window.configure(bg='#132c33')
    tk.Label(add_window,bg='#132c33',fg="white").grid(row=0, column=0, columnspan=2)
    tk.Label(add_window, text="Course Title:",bg='#132c33',fg="white").grid(row=1, column=0)
    ...
    tk.Label(add_window, text="Subject Code:",bg='#132c33',fg="white").grid(row=3, column=0, padx=20)
...
def display():
    ...
    display_window.title("Course Database")
    ...
    cursor = connn.execute("SELECT rowid,* FROM course;")
    i = 0
    for row in cursor:
        table.insert('', i, text="Course " + str(row[1]), values=(row[2], row[3], row[4]))
        i = i + 1
    table.pack()
    connn.close()
...
def update():
    update_window = tk.Tk()
    update_window.title("Update Course Details")
    update_window.configure(bg='#132c33')
    tk.Label(update_window, text="Insert Code of Course :",bg='#132c33',fg="white").grid(row=0, column=0, sticky="W", padx=10, columnspan=2)
    ...
    tk.Label(update_window, text="Title:",bg='#132c33',fg="white").grid(row=3, column=0, sticky="W", padx=10, pady=10)
    ...
    tk.Label(update_window, text="Subject Code:",bg='#132c33',fg="white").grid(row=5, column=0, sticky="W", padx=10, pady=10)
    ...
    def fillup():
        ...
        try: 
            connn = sqlite3.connect("ACME.db")
            cursor = connn.execute("SELECT * FROM course where code='"+sid+"';")
            connn.commit()
            for row in cursor:
    ...
    def submit():
        ...
        flag=True        
        try:        
            scon.execute("UPDATE course SET title = '" + title + "',section = '" + section + "', code = '" + code +"';")
            scon.commit()
        except sqlite3.Error as er:
            messagebox.showinfo("Error", "Something went wrong")
            flag=False        
...
def delete():
    ...
    tk.Label(delete_window, text="Enter Subject Code whose details are to be removed:",bg='#132c33',fg="white").grid(row=0, column=0, padx=10, pady=10)
    ...
    def submit():
        ...
        flag=True
        try:        
            dcon.execute("DELETE FROM course WHERE code = '" + code+"';")
            dcon.commit()
            dcon.execute("VACUUM;")
            dcon.commit()
        except sqlite3.Error as er:
...
#finding Course details gui

def search():
    search_window = tk.Tk()
    search_window.title("Search Course Details")
    search_window.configure(bg='#132c33')
    tk.Label(search_window, text="Enter the Subject Code of Course whose details are to be searched:",bg='#132c33',fg="white").grid(row=0, column=0,
                                                                                                     padx=10, pady=10)
    ...
    details.heading("one", text="Title")
    ...
    def submit():
        ...
        cursor = fcon.execute("SELECT rowid,* from course WHERE code = '" + code + "';")
        fcon.commit()
